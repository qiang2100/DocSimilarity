...
public class INSGrow {
	...
	private ArrayList<Integer>[] pos = new ArrayList[MAXE]; //  index of event position: pos[i].get(j) = the jth position of event i,é”Ÿæ–¤æ‹·ié”Ÿä¾¥ç¢‰æ‹·jé”Ÿæ–¤æ‹·ä½�é”Ÿæ–¤æ‹·
	
	private HashMap<Integer, PIndex>[] pst = new HashMap[MAXN]; // pst[i].get(j),positions of j in sequence i
	private int[] res = new int[MAXL]; // reversed index of pos[][]// res[i],iä½�é”Ÿæ–¤æ‹·é”Ÿè§’é�©æ‹·ä½�é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”ŸèŠ‚ç¢‰æ‹·å‰�é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿå�«çš„ç¬¬ç¡·æ‹·é”Ÿæ–¤æ‹·ä½�é”Ÿæ–¤æ‹·, é”Ÿæ–¤æ‹·0é”Ÿæ–¤æ‹·å§‹
	private int[] ind = new int[MAXL]; // index of sequence number of each position, ind[i], iä½�é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”ŸèŠ‚ç¢‰æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹· 
	private int[] next = new int[MAXL]; // index of event position: next[i] = the next position of event db[i] in this sequence (or =-1 if no one left), iä½�é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿå�«ç¢‰æ‹·é”Ÿæ–¤æ‹·ä¸€é”Ÿæ–¤æ‹·ä½�é”Ÿæ–¤æ‹·
	
	private int nid; // number of different events,é”Ÿæ–¤æ‹·æ¯�é”Ÿæ–¤æ‹·é”Ÿï¿½
	
	private int[] et = new int[MAXE]; // et[i] = the ith event
	private HashMap <Integer, Integer> id; // index of event ID, é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·é”Ÿçµ dé”Ÿæ–¤æ‹·é”Ÿæ–¤æ‹·
		...
		for(x=0;x<pat.getP().size();x++)
		{
			...
			while(itp.hasNext())
			{
				...
				if (newitm==pat.getP().get(x))
				{
					...
					npat.clear();
					continue; // æ‰§é”Ÿæ–¤æ‹·itp.hasNext()
				}
				for (l=x;l<pat.getP().size();l++) // <p0ä½�é”Ÿæ–¤æ‹·
				{
					nnpat.clear(); // 
					for (i=0;i<npat.size();)
					{
					...
					if (i<npat.size())
						break;
					npat=(ArrayList)nnpat.clone();// ä»€ä¹ˆé”Ÿæ–¤æ‹·æ€�
				}
				if (l==pat.getP().size())
				{
					nclosed=1;
...
//		show(pat);
	...
	public void printHashMap() 
	{
		try
		{
			...
			FileWriter fwText = new FileWriter("C:/Users/jipeng/Desktop/Qiang/KNN/OHSUMED/2_closePatt.txt");
