...
public class DUC2002_Best {
	...
	public String readTextFromFile(String fileName)
	{
		...
		try 
		{
			...
			for(int i=2; i<sentences.length-1; i=i+2)
				text += sentences[i].substring(0, sentences[i].length()-3);
		...
		for(int i=0; i<dirList.length; i++)
		{
			...
			for(int j=0; j<twoDirList.length; j++)
			{
				if(twoDirList[j].equals("200e"))
				{
					...
					String newName = dirList[i].substring(0,dirList[i].length()-1)+"_best_" + flag+"_.spl";
					
					if(flag == 1)
						flag = 2;
					else
						flag = 1;
					File writeFile = new File(outputDir+"\\"+dirList[i].substring(0,dirList[i].length()-1)+"\\"+"peers\\");		
					if(!writeFile.exists())
					{		 
						writeFile.mkdirs();		
					} 
	...
	public static void main(String[] args)
	{
		...
		try
		{
			String dirName = "F:/Êý¾Ý¼¯/DUC2002/summaries";
			
			DUC2002_Best duc = new DUC2002_Best();
